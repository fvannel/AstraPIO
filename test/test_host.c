...
static int transfer(void *ctx, const uint8_t tx[4], uint8_t rx[4]) {
    ...
    memset(rx, 0, 4);
    if (tx[0] == 2) {
        if (a == 0x0b) m->memory[m->reg[0x0a]] = v;
        else m->reg[a] = v;
    } else {
        assert(tx[0] == 3);
        v = a == 0x0b ? m->memory[m->reg[0x0a]] : m->reg[a];
        if (m->corrupt && a == 0x0b) v ^= 1;
...
int main(void) {
    ...
    uint16_t code[48]; for (unsigned i=0; i<48; i++) code[i] = (uint16_t)(0xa000+i);
    m.reg[0]=0x5049; m.reg[1]=0x0200;
    assert(pio_probe(&d)==0);
    assert(pio_load(&d, 1, code, 48)==0);
    assert(!memcmp(m.memory+64, code, sizeof code));
    assert(m.reg[3]==0 && m.reg[4]==2 && m.reg[0x2f]==0);
    assert(pio_set_mask(&d, 0, 0x100)==PIO_EINVAL);
    assert(pio_set_mask(&d, 1, 0x100)==0);
    assert(pio_run(&d, 3)==0 && m.reg[3]==3);
    m.reg[0x16]=16; assert(pio_push(&d,0,0x55)==PIO_EFULL);
