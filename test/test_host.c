...
int main(void) {
    ...
    assert(m.reg[3]==0 && m.reg[4]==1 && m.reg[0x0a]==0);
    assert(pio_set_entry(&d,0,7)==0 && m.reg[0x11]==7);
    assert(pio_set_entry(&d,1,7)==PIO_EINVAL && m.reg[0x21]==0);
    assert(pio_set_mask(&d,0,0x4000)==PIO_EINVAL);
    assert(pio_set_mask(&d,0,0x3FFF)==0);
    assert(pio_set_mask(&d,1,0x100)==PIO_EINVAL);
    assert(pio_run(&d,3)==PIO_EINVAL);
    assert(pio_run(&d,1)==0 && m.reg[3]==1);
