...
class ToolsTest(unittest.TestCase):
    ...
    def test_reject_bad_programs(self):
        for source in ("", "OUT 1", "LDI -1", "LDI 256", "JMP 48", "JMP missing",
    ...
    def test_extended_instructions(self):
        self.assertEqual(assemble("PULL\nPUSH\nSET 6,1\nOUTBIT 6\nINBIT 12\n"
                                  "LDX 0\nDJNZ 47\nSIGNAL\nAWAIT\nCLR_EVENT\nJBIT 47\nRECV\nIN 1"),
                         [0xF000, 0xF100, 0xF261, 0xF360, 0xF40C, 0xF500, 0xF62F,
                          0xF700, 0xF710, 0xF720, 0xF82F, 0xF900, 0x2100])
        for source in ("SET 7,1", "OUTBIT 7", "INBIT 13", "LDX 16", "DJNZ 48", "JBIT -1"):
            with self.subTest(source=source), self.assertRaises(ValueError):
                assemble(source)

    def test_examples_assemble(self):
        for path in (Path(__file__).resolve().parents[1] / "examples").glob("*.pio"):
            with self.subTest(path=path):
                self.assertLessEqual(len(assemble(path.read_text())), 48)
    ...
    def test_frames(self):
        ...
        self.assertEqual(program_frames(1, [0xE000]),
                         [bytes.fromhex("02 2f 00 00"), bytes.fromhex("02 50 e0 00")])
        self.assertEqual(program_frames(1, [0] * 48)[-2:],
                         [bytes.fromhex("02 0a 00 6f"), bytes.fromhex("02 0b 00 00")])
