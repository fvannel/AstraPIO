...
async def lpc_counter_on_demand_replaces_prefix_and_relays_tail(dut):
    ...
    assert await host.read(0x01) == ABI
    assert FEATURES == 4 and await host.read(0x1A) == 4
    assert await host.read(0x60) == 0x5449

    # The NEW OUTMSB instruction keeps toggling uio1 during all WS2812 traffic.
    # LDI 2; DIR; LDI 128; OUTMSB 1; XOR 128; JMP 3 (ABI-6 words).
    # No data shift is permitted: each XOR must invert the next observed bit.
    await host.write(0x10, 2)
    for address, word in enumerate((0x002, 0x301, 0x080, 0x3E1, 0x280, 0x313)):
    ...
    source_names = ("project.v", "pio_spi.v", "pio_single_core.v", "pio_program.v", "pio_fifo.v", "pio_timed.v")
    source_hashes = {name: hashlib.sha256((root/"src"/name).read_bytes()).hexdigest() for name in source_names}
    netlist = root/"test/gate_level_netlist.v"
    netlist_sha = hashlib.sha256(netlist.read_bytes()).hexdigest() if os.environ.get("ASTRA_WS_MODE") == "gl" else None
    report = {"status": "PASS", "mode": os.environ.get("ASTRA_WS_MODE", "unspecified"),
              "source_commit": source_commit, "source_sha256": source_hashes,
              "routed_netlist_sha256": netlist_sha, "expected_abi": ABI,
              "concurrent_pio_program": "OUTMSB_toggle",
