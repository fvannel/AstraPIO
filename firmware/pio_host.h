...
/* ABI v5 or exact OUTMSB-only ABI v6 (capability register 1A == 4).
 * Ten-bit program words in uint16_t containers (upper bits zero).
 * All context arguments must be zero; run mask must be 0 or 1.
 * Reload resets the sole context and its queues. Probe rejects v3/v4 and
 * unknown ABI-6 variants. OUTMSB code is rejected on ABI-5 before stopping it. */
