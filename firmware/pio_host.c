...
int pio_probe(pio_device *d) {
    ...
    return rc ? rc : value == 0x0200 ? PIO_OK : PIO_EABI;
}
int pio_stop(pio_device *d) { return pio_write(d, 3, 0); }

int pio_load(pio_device *d, unsigned c, const uint16_t *words, size_t count) {
    uint16_t value;
    int rc;
    if (c > 1 || !words || count == 0 || count > 48) return PIO_EINVAL;
    if ((rc = pio_stop(d))) return rc;
    if ((rc = pio_write(d, 4, (uint16_t)(1u << c)))) return rc;
    if ((rc = pio_write(d, (uint8_t)(0x1f + 16*c), 0))) return rc;
    for (size_t i = 0; i < count; ++i) {
        if ((rc = pio_write(d, 0x0a, (uint16_t)(64*c+i)))) return rc;
        if ((rc = pio_write(d, 0x0b, words[i]))) return rc;
        if ((rc = pio_read(d, 0x0b, &value))) return rc;
...
int pio_push(pio_device *d, unsigned c, uint8_t byte) {
    ...
    if ((status & 31) == 16) return PIO_EFULL;
...
int pio_push_many(pio_device *d, unsigned c, const uint8_t *bytes, size_t count, size_t *sent) {
    ...
    *sent = 0;
    ...
    if ((rc = pio_read(d, (uint8_t)(0x16 + 16*c), &status))) return rc;
    unsigned level = status & 31;
    if (level > 16) return PIO_EVERIFY;
    size_t available = 16 - level;
