...
PDK complets, traces et sorties intermédiaires sont déplacées vers la Corbeille
lors du nettoyage, pas intégrées au dossier final. Les registres JSON conservent
les identifiants des builds et les raisons de refus sans conserver leurs gigaoctets.

Ne pas confondre le `main` GitHub historique avec le SHA accepté dans le shuttle.
Le nettoyage n'effectue aucun push, lancement CI, merge ou nouvelle soumission.
